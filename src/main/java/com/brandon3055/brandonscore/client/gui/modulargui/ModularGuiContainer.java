...
public abstract class ModularGuiContainer<T extends Container> extends GuiContainer implements IModularGui<ModularGuiContainer> {
    ...
    public boolean isMouseOverSlot(Slot slotIn, int mouseX, int mouseY) {
        return super.isMouseOverSlot(slotIn, mouseX, mouseY) && !manager.isAreaUnderElement(slotIn.xPos + guiLeft(), slotIn.yPos + guiTop(), 16, 16, 100);
    }

    @Override
    public void drawSlot(Slot slotIn) {
        if (manager.isAreaUnderElement(slotIn.xPos + guiLeft(), slotIn.yPos + guiTop(), 16, 16, 100)) {
            return;
        }
