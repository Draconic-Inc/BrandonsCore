...
public class MGuiList extends MGuiElementBase<MGuiList> implements IScrollListener {
    ...
    public MGuiList(int xPos, int yPos) {
        super(xPos, yPos);
    }

    public MGuiList(int xPos, int yPos, int xSize, int ySize) {
    ...
    protected void initScrollBar() {
        ...
        scrollBar = new MGuiScrollBar(xPos() + xSize() - 10, yPos() + 1, 10, ySize() - 2);
        addChild(scrollBar);
...
//        GL11.glEnable(GL11.GL_SCISSOR_TEST);
        ...
        double hs = (double) minecraft.displayHeight / screenHeight;
    ...
    public MGuiList sortEvenSpacing(boolean compress) {
        ...
        if (remainingSpace >= 0) {
            ...
            for (MGuiListEntry entry : listEntries) {
                ...
                entry.translate(0, (int) (y + offsetShare));
                y += offsetShare * 2;
            }
        }
        else {
