...
public class ModularGuiProperties implements IGuiProperties {
	...
	@Nullable
	public static ModularGuiProperties create(IModularGui<?> modularGui) {
		Screen screen = modularGui.getScreen();
		if (screen.width <= 0 || screen.height <= 0) {
			return null;
		}
		int x = modularGui.guiLeft();
		int y = modularGui.guiTop();
		int width = modularGui.xSize();
		int height = modularGui.ySize();
